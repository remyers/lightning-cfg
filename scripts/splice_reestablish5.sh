...
echo == Test: "Disconnection after exchanging tx_signatures and one side sends commit_sig for channel update"
alice-eclair-cli splicein --channelId=$CHANNEL_ID --amountIn=15007 >& /dev/null
sleep 10
alice_wait_for_state "NORMAL" 
...
export INVOICEA1=$(bob-clightning-cli invoice 15007000 test1 test1 | jq -r .bolt11)
echo Invoice Id: $(alice-eclair-cli payinvoice --invoice=$INVOICEA1)
check_log "Not sending our commit sigs for interop test: Disconnection after exchanging tx_signatures and one side sends commit_sig for channel update" 1
sleep 10
alice_wait_for_state "NORMAL" 
bob_wait_for_state "CHANNELD_AWAITING_SPLICE"
check_funds 484993000 15007000
...
check_log "IN msg=RevokeAndAck" 2
...
bob_wait_for_state "CHANNELD_NORMAL"
check_funding_txid
check_funds 500000000 15007000
