...
# create encrypted perf legacy wallet
btc-cli createwallet "perf" false false "foo" false false
btc-cli --rpcwallet=perf walletpassphrase "foo" 1000000

# create eclair/miner descriptor wallet
btc-cli createwallet "eclair"

MINER=$(btc-cli --rpcwallet=eclair getnewaddress)
btc-cli generatetoaddress 9 $MINER
...
echo "move 1000 UTXOs over 100,000 new blocks"
for i in {1..100000} 
do
    echo "create/move 1000 UTXOs"
    ADDR=$(btc-cli --rpcwallet=perf getnewaddress)
    MANY=\"$ADDR\":1
    for i in {1..1000}
    do
...
# btc-cli -rpcwallet=perf createrawtransaction [{"txid":"16592e51c066afbd7d8a734ca9977cf1c4b6c934be5dd095e69ade6be49b9808","vout":0}] [{"bcrt1qcddszume0dv4sn75hjx0fwdacs22fypre6dcj0":1}]
